...
var yValueGuy = window.getComputedStyle(targetRender)['background-position'].split(' ')[1];
...
function bannerIsDragged(event){ 
	...
	targetRender.style['background-position'] = -newValue*imgWidth + 'px' + ' ' + yValueGuy;
	...
	targetObj.style['background-color'] = 'rgba(0, 0, 0, ' + bgOpacity + ')';
...
function bannerContactEnd(){ 
	var elem = targetObj,
		...
		origVal  = parseInt(targetRender.style['background-position'].split(' ')[0]),
		finalVal = 0,
	...
	function step(timestamp) {
        ...
        targetRender.style[property] = newVal + unitOfMeasurement + ' ' + yValueGuy;
